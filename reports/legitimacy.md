...
- Encompassing, 2019-22: margin = -0.88 + 0.725 x line + 0.321 x model (model t = 1.90). A model weight above zero with t past 2 means the line does not already contain what the model knows.
...
- Placebo: the real 4+ record is 119-72 (62.3%). Shuffling the model's lines within each week 2,000 times gives a mean of 51.2% and a 95th percentile of 53.1%; 0.00% of shuffles reach the real record.

- Bootstrap on the 191 real 4+ flags: 90% interval for the win rate 57.1% to 67.5%; 0.4% of resamples fall under the 52.4% break-even.
...
- Leave-one-season-out 4+ records (each season dropped in turn): 95-60, 96-63, 101-62, 105-52, 113-69, 98-63, 106-63. No single season carries the record if every one of these stays above break-even.
