    ...
    for r in rr.itertuples():
        ...
        if grp == "QB":
            ...
            if g is not None:
                rec = last8(g)
                if len(rec):
                    v, n = pv_def.value(r.gsis_id, "defender", season, week); pr = pv_def.prior("defender", season)
                    ...
                    row.update({"games": int(len(rec)), "plays_per_game": round(snap_share, 1), "share": round(min(share, 1.0), 3), "epa_per_play": round(v, 4), "value_above_replacement": round((v - pr) * min(share, 1.0), 4), "basis": "Impact -EPA per defensive snap x snap share"})
        elif grp == "OL":
