...
# Defender value by group (24 Sep 2026). Two yardsticks chose these: experiments/cb_value.py (a corner's rating
# before a season against his coverage that season) and experiments/allpro_check.py (where each season's AP All-Pros
# land in our list at season's end, both windows; reports/allpro_check.csv). Per group, the variant best on 2019-22
# among those better than the old one on both windows:
#   CB    coverage per TARGET (yards saved and interceptions, in EPA, over targets + 150) at a typical corner's
#         target volume; 0.99 a game, no season fade
#   S     the same coverage per target, plus half of his run stops and pass rush per snap
#   IDL   the same shape as S (coverage is small for linemen; run stops and rush carry it), 0.99 a game
#   EDGE  every part per snap (coverage, rush, run stops, forced fumbles), 0.98 a game and 0.9 a season back
...
def role_rates(dg: pd.DataFrame, roles: dict, season: int, week: int) -> tuple[dict, dict]:
    """Every defender in a ROLE_SPEC group: his rate in EPA per snap as of (season, week), and each group's replacement
    level (25th percentile of its players with 300+ weighted snaps). The coverage-per-target rates are scaled by the
    league's corner targets per snap."""
    d = dg[(dg.season >= 2018) & ((dg.season < season) | ((dg.season == season) & (dg.week < week)))].sort_values(["season", "week"])
    if not len(d):
        return {}, {}
    cbs = d[d.player_id.map(roles) == "CB"]; t_snap = float(cbs.targets.sum() / cbs.plays.sum()) if len(cbs) else 0.09
    cov = DEF_W["cov_yds"] * d.cov_yds.values + DEF_W["cov_int"] * d.cov_int.values
        ...
        if grp == "QB":
            ...
            if g is not None:
                ...
                if len(rec):
                    ...
                    row.update({"games": int(len(rec)), "plays_per_game": round(snap_share, 1), "share": round(min(share, 1.0), 3), "epa_per_play": round(v, 4), "value_above_replacement": round((v - pr) * min(share, 1.0), 4), "basis": {"CB": "Coverage per target (yards saved and interceptions, in EPA) at a typical corner's target volume, x snap share, against CB replacement",
                                                                                                                                                                                                                                          "S": "Coverage per target plus half his run stops and pass rush per snap, x snap share, against S replacement", "IDL": "Run stops and pass rush per snap (half weight) plus coverage per target, x snap share, against IDL replacement",
                                                                                                                                                                                                                                          "LB": "Coverage, pass rush, run stops and forced fumbles per snap plus 0.75 EPA per credited play (tackle, sack, pass defended), x snap share, against LB replacement"}.get(role, f"Defensive EPA saved per snap (coverage, pressures, interceptions, run stops) x snap share, against {role} replacement"), "def_role": role})
