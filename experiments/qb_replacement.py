...
for lvl in LEVELS:
    # the level goes in through the parameters (build_features passes qb_prior to QBRatings explicitly since 23 Sep 2026,
    # so patching the constructor's default, as this script first did, no longer changes anything)
    f = M.with_trends(R.build_features({**R.DEFAULT, "qb_prior": lvl}, tg=tg, games=games, qb=qb)); r = both(f)
...
out = os.environ.get("OUT", "reports/qb_replacement.csv")
